...
// Master side OLED display (WPM, current key, layer)
...
// Slave side OLED display (Lily58 logo)
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  if (record->event.pressed) {
    #ifdef OLED_ENABLE
    // We don't need to call add_keylog since we're using a simpler approach
    // Just using the WPM counter and mod key display
    #endif
  }
  return true;
}